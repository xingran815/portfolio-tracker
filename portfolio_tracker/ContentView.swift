...
struct ContentView: View {
    ...
    // ViewModels - using State for @Observable macro
    ...
    var body: some View {
        NavigationSplitView {
            ...
            PortfolioDetailView(
                ...
                portfolio: listViewModel.selectedPortfolio
            )
        } detail: {
        ...
        .sheet(isPresented: $showingSettingsWindow, onDismiss: {
            // Check if we should switch to real API after settings closes
            Task {
                await chatViewModel.switchToRealService()
            }
        }) {
            SettingsWindow()
        }
