...
final class PortfolioDetailViewModel {
    ...
    /// The portfolio being displayed (CoreData object - for mutations)
    var portfolio: Portfolio?
    
    /// View-safe portfolio data (for display)
    var portfolioViewData: PortfolioViewData?
    
    /// Positions sorted by value (CoreData objects - for mutations)
    var positions: [Position] = []
    
    /// View-safe position data (for display)
    var positionViewData: [PositionViewData] = []
    ...
    func setPortfolio(_ portfolio: Portfolio?) {
        ...
        self.portfolioViewData = portfolio.map { PortfolioViewData.from($0) }
        loadPositions()
        driftAnalysis = nil
        rebalancePlan = nil
    }
    ...
    func loadPositions() {
        ...
        // Create view-safe data
        let totalValue = portfolio.totalValue
        positionViewData = positions.map { PositionViewData.from($0, totalPortfolioValue: totalValue) }
    }
