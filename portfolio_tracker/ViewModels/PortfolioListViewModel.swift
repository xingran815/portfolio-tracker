...
// Re-export PortfolioSnapshot for use in Views
typealias PortfolioViewSnapshot = PortfolioSnapshot
...
final class PortfolioListViewModel {
    ...
    /// Returns a snapshot of the selected portfolio for safe cross-actor access
    var selectedPortfolioSnapshot: PortfolioSnapshot? {
        guard let portfolio = selectedPortfolio else { return nil }
        return PortfolioSnapshot.from(portfolio)
    }
